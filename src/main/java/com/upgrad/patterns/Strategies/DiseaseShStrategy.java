...
public class DiseaseShStrategy implements IndianDiseaseStat {
    ...
    public String GetActiveCount() {
        try {
            ...
            if (response != null) {
                double activeCount = Math.round(response.getCases());
                return String.valueOf((int) activeCount); // Convert to integer and return as String
            }
        } catch (Exception e) {
            logger.error("Error fetching disease count from DiseaseSh source: " + e.getMessage());
        }
        return null;
    }
